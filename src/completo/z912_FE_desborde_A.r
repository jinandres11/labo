...
AgregarVariables  <- function( dataset )
{
  ...
  #variables creadas en funcion de  campos monentarios sobre la edad o la antiguedad 
  dataset[ , JIA_ctrx_quarter_sobre_edad := ctrx_quarter/cliente_edad ]
  dataset[ , JIA_rent_sobre_antig        := mrentabilidad / cliente_antiguedad]
  dataset[ , JIA_rent_sobre_edad         := mrentabilidad / cliente_edad]
  dataset[ , JIA_rent_anio_sobre_edad    := mrentabilidad_annual / cliente_edad]
  dataset[ , JIA_act_pas_sobre_edad      := (mactivos_margen + mpasivos_margen) / cliente_edad]
  dataset[ , JIA_limcomp_sobre_edad      := (Visa_mlimitecompra + Master_mlimitecompra) / cliente_edad]
  dataset[ , JIA_limcomp_sobre_antig     := (Visa_mlimitecompra + Master_mlimitecompra) / cliente_antiguedad]
  dataset[ , JIA_montoinv_vs_edad        := montos_inv_y_plazofijo / cliente_edad]
  
  #Algunas variables que me permiten medir la confianza del cliente en el banco, viendo saldos en cta cte, ca y plazo fijo.
  
  dataset[ , JIA_total_CC             := mcuenta_corriente + mcuenta_corriente_adicional]
  dataset[ , JIA_total_CA             := mcaja_ahorro + mcaja_ahorro_adicional + mcaja_ahorro_dolares]
  dataset[ , JIA_totalCC_sobre_cantCC := JIA_total_CC / ccuenta_corriente]
  dataset[ , JIA_totalCA_sobre_cantCA := JIA_total_CA / ccaja_ahorro]
  dataset[ , JIA_ctasaldo_sob_cp      := mcuentas_saldo / cproductos] # relacion entre el total de saldo de cuenta y la cantida de productos
  dataset[ , JIA_gan_bco_sobre_prod   := (mactivos_margen + mpasivos_margen + mcomisiones) / cproductos] #es la ganancia que esta teniendo el banco sobre el total de producto que tiene el cliente
  dataset[ , JIA_saldo_sobre_movi     := (mcuentas_saldo) / ctrx_quarter] 
  dataset[ , JIA_movi_sobre_pro       := (ctrx_quarter) / cproductos] 
